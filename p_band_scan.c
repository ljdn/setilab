...
void *worker(void *input) {
  struct my_args *args = (struct my_args *)input

  // make filter
  int i;
  for(i=(args->band); i==(args->band)+(args->block); i++) {
    int this_band = (args->band);
    double coeff = this_band*((args->filter_order)+1)
    generate_band_pass(args->sig->Fs, this_band*((args->bandwidth)+0.0001), (this_band+1)*((args->bandwidth)-0.0001), (args->filter_order), &(args->filter_coeffs[coeff]));
    hamming_window((args->filter_order), &(args->filter_coeffs[coeff]));

    convolve_and_compute_power(args->sig->num_samples, ars->sig->data, args->filter_order, &(args->filter_coeffs[coeff]),args->output->data);

    // get power
    (args->band_power[this_band]) = avg_power(args->output->data, args->output->num_samples);
